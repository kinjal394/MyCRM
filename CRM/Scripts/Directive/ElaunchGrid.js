                    ...
                    link: function (scope, element, attrs) {
                        ...
                        scope.renderGrid = function (scope) {
                            ...
                            if (scope.onrowdblclick) {
                                tempText += '<tr data-ng-repeat="row in $data" ng-click="RowClick(row)"  ng-dblclick="RowDblClick(row)">'
                                    + ' <td ng-repeat="col in columnInfo.columnsInfo" ><div ng-if="col.cellTemplte" ng-bind-html="col.cellTemplte(this,row)" compile-template></div><span ng-if="col.cellTemplte == undifined">{{row[col.field]}}</span></td>'
                                    + ' </tr>'
                            }
                            else {
                                tempText += '<tr ng-repeat="row in $data">'
                                    + ' <td ng-repeat="col in columnInfo.columnsInfo" ><div ng-if="col.cellTemplte" ng-bind-html="col.cellTemplte(this,row)" compile-template></div><span ng-if="col.cellTemplte == undifined">{{row[col.field]}}</span></td>'
                                    + ' </tr>'
                            }
                            + ' </table>'
                            ...
                            tempText += '</div>'
                                ...
                                + '</div>'
                                + '</div>';
                            //              var extra = '<div class="dropdown"><button id="dLabel" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true" class="btn btn-primary"><span class="caret"></span></button>'
                            //+ '<ul class="dropdown-menu" aria-labelledby="dLabel"><li data-ng-repeat="col in columnInfo.columnsInfo"><input type="checkbox" ng-change="SetColumnVisibility(col,$index)" ng-model="col.show"/> {{col.title}}</li></ul>';
                            //  var extra = '<div style="margin-bottom: 20px"><label class="checkbox-inline" data-ng-repeat="col in columnInfo.columnsInfo"><input type="checkbox" ng-change="SetColumnVisibility(col,$index)" ng-model="col.show"/> {{col.title}}</label></div>';
